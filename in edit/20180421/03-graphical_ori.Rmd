...
Visualization is the process of  representing data graphically and  interacting with these representations. The main objective is to gain insight into the data (Reference: <http://researcher.watson.ibm.com/researcher/view_group.php?id=143>
...
We will be using a dataset named `cholest1.dta` which is in Stata format. 

```{r}
library(haven)
cholest <- read_dta('cholest1.dta')
cholest
```
...
You can always personalize the graphical parameters such as parameters for Fonts, Colours, Lines and Symbols. Not enough time to go through one by one. Can try on your own by visiting this link <http://www.statmethods.net/advgraphs/parameters.html>
...
To use **lattice** package, we first load the package using `library()`
...
Plot a histogram for variable `gcsescore` and label the x axis
